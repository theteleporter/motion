...
/**
 * DOMAnimation is a holding class for either a native or JS animation.
 *
 * It can be intitalised without resolved keyframes. Once keyframes are
 * resolved, it will decide which type of animation to initialise.
 *
 * Keyframes will be resolved asynchronously, unless the animation is interacted
 * with in such a way that requires a resolved animation. In which case
 * all pending keyframes will be resolved synchronously.
 */
export class DOMAnimation implements AnimationPlaybackControls {
    /**
